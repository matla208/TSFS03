function [costToGo,nextX,nextY]=dynProg2D(funName,tVec,discX,discY,J_T);
  global V_z
  ...
  for t=(length(tVec)-1):-1:1,
    ...
    for ii=1:length(discX),
      for jj=1:length(discY),
        ...
        currCost=funName([tVec(t) tVec(t+1)],discX(ii),discX,discY(jj),discY)+squeeze(costToGo(t+1,:,:));
